...
$redis->auth('admin888');
$redis->select('0');
$grouptime = $redis->lrange('grouptime3',0,-1);
if($grouptime[0]>time()){
	$redis->close();exit();
}

$list = $redis->lrange('groupsend3', 0, -1);
if ($list) {
    $i = 0;
    $len = $redis->lLen('groupsend3');
    while (true) {
        if ($i >= $len) {
	    $redis->lpop('grouptime3'); 
            $redis->close();
            file_put_contents('/usr/local/etc/outqueuecok3.txt', 1);
            @unlink('/usr/local/etc/groupdata3.txt');
            exit();
        }
        $openid = $redis->lpop('groupsend3');
        if ($openid) {
            //send-msg
...
function send_template_msg($openid)
{
    ...
    $opt_json = json_decode(file_get_contents('/usr/local/etc/groupdata3.txt'));
