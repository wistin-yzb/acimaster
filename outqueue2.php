...
$redis->auth('admin888');
$redis->select('0');
$grouptime = $redis->lrange('grouptime2',0,-1);
if($grouptime[0]>time()){
	$redis->close();exit();
}

$list = $redis->lrange('groupsend2', 0, -1);
if ($list) {
    $i = 0;
    $len = $redis->lLen('groupsend2');
    while (true) {
        if ($i >= $len) {
	    $redis->lpop('grouptime2');
            $redis->close();
            file_put_contents('/usr/local/etc/outqueuecok2.txt', 1);
            @unlink('/usr/local/etc/groupdata2.txt');
            exit();
        }
        $openid = $redis->lpop('groupsend2');
        if ($openid) {
            //send-msg
...
function send_template_msg($openid)
{
    ...
    $opt_json = json_decode(file_get_contents('/usr/local/etc/groupdata2.txt'));
